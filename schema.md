...
| Samsung TV Network Control | `infrastructure/runbooks/wol-samsung-tv.md` | Full network control of Samsung TV (hostname "Samsung", IP 192.168.50.175 DHCP, MAC 54:3A:D6:5D:B0:EC) via new `tv_control.py` script (volume prioritized, WoL power-on, key sending, pairing with token in ~/.config/samsung-tv-token.json); builds on prior DHCP/WoL success (verified 2026-05-30); includes install (sheepsoc conda + samsungtvws/wakeonlan), usage, prerequisites (wired Ethernet, WoL + Network Remote enabled), how it works, troubleshooting, topology links. |
