...
| Samsung TV Network Control | `infrastructure/runbooks/wol-samsung-tv.md` | Full network control of Samsung TV (hostname "Samsung", IP 192.168.50.175 DHCP, MAC 54:3A:D6:5D:B0:EC) via `tv_control.py` script (volume prioritized, WoL power-on via wakeonlan, raw keys, **--youtube-search "QUERY"** using samsungtvws.run_app with YouTube ID 111299001912 + navigation keys to search field + 15x `KEY_BACKSPACE` clear loop + per-character loop (`for char in query.lower(): if isalpha send KEY_{upper}(char) else KEY_SPACE`; 0.25s sleeps) replacing `send_text()` per observation "send_text is not working. I can see the cursor move" (types letters directly when focused). Re-tested with "Try not to laugh". Token saved in ~/.config/samsung-tv-token.json); builds on prior DHCP/WoL/volume success (verified 2026-05-30); includes install, expanded usage (current char-keys impl), prerequisites (wired Ethernet, WoL + Network Remote/Access Notification), How It Works (clear + char loop), troubleshooting, topology links. Wiki is living record of cursor-move + per-char KEY_* method. |
