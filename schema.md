...
| Samsung TV Network Control | `infrastructure/runbooks/wol-samsung-tv.md` | Full network control of Samsung TV (hostname "Samsung", IP 192.168.50.175 DHCP, MAC 54:3A:D6:5D:B0:EC) via `tv_control.py` script (volume prioritized, WoL power-on via wakeonlan, raw keys, **--youtube-search "QUERY"** using samsungtvws.run_app with YouTube ID 111299001912 + send_key navigation + send_text, token pairing in ~/.config/samsung-tv-token.json); builds on prior DHCP/WoL success (verified 2026-05-30); tested via `conda run -n sheepsoc` (no errors, TV responded); includes install (sheepsoc conda + samsungtvws/wakeonlan), expanded usage, prerequisites (wired Ethernet, WoL + Network Remote/Access Notification enabled), how it works, troubleshooting, topology links. |
