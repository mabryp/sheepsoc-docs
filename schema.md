...
| Samsung TV Network Control | `infrastructure/runbooks/wol-samsung-tv.md` | Full network control of Samsung TV (hostname "Samsung", IP 192.168.50.175 DHCP, MAC 54:3A:D6:5D:B0:EC) via `tv_control.py` script (volume prioritized, WoL power-on via wakeonlan, raw keys, **--youtube-search "QUERY"** using samsungtvws.run_app with YouTube ID 111299001912 + navigation keys to search + reliable 15x `KEY_BACKSPACE` loop + sleep(0.1) before `send_text()` to clear field (reverted from `KEY_CLEAR` after user feedback "KEY_CLEAR didn't clear field"; tested KEY_CLEAR but insufficient per "clear the search before it runs a search" request; back to original reliable impl.; navigation/launch unchanged; token saved in ~/.config/samsung-tv-token.json); builds on prior DHCP/WoL success (verified 2026-05-30 with clear-search test + reversion); tested via `conda run -n sheepsoc` (no errors, token present); includes install (sheepsoc conda + samsungtvws/wakeonlan), expanded usage, prerequisites (wired Ethernet, WoL + Network Remote/Access Notification enabled), how it works (incl. clear step), troubleshooting, topology links. |
