...
| **Logstash** | `logstash.service` | 5514/udp | Syslog receiver from ASUS router, OPNsense, and SAN01 Synology NAS · ships to **Elastic Cloud 9.4.0** (migrated 2026-06-29) · data streams: `logs-syslog.opnsense-default` (confirmed), `logs-syslog.asus-default` (confirmed 2026-06-29), `logs-syslog.synology-default` (confirmed 2026-06-29), `logs-syslog.other-default` · see [Log Shipping](platforms/log-shipping.md) | up |
...
| **Ollama** | `ollama.service` | 11434 | Local LLM inference (uses RTX 5060 Ti) · **0.30.10** (upgraded 2026-06-26) · exposes Ollama-native API, OpenAI-compatible `/v1`, and Anthropic Messages `/v1/messages` (≥0.14.0) · 14 models at `~/.ollama` · single-request concurrency (`OLLAMA_NUM_PARALLEL=1`) and a 16384-token default context (`OLLAMA_CONTEXT_LENGTH`), both retuned 2026-07-02 for the 16 GB VRAM ceiling · `claude-ollama` wrapper enables Claude Code to use local models — see [Ollama](platforms/ollama.md) | up |
...
| **Sheepsoc Lab Hub** | `labhub-web.service` (web UI) · `labhub-collect.timer` + `labhub-collect.service` (collector, 3 min) · `labhub-synth.service` + `labhub-synth.timer` (**staged only — not yet installed**) | 8800 | "State of the Lab" dashboard — health tiles, Ollama summary blurb, drift detection, and daily security brief · **reads this Service Catalog as expected-state source of truth** · Phase 3b reads Elastic Cloud 9.4.0 · daily brief delivered to Element via Matrix Bot outbox · LAN-only (no Tailscale) · runs in `labhub` conda env (Python 3.12) — see [Lab Hub](platforms/lab-hub.md) | up |
...
| SAN01 DSM | `http://192.168.50.165:5000` (HTTP) · `https://192.168.50.165:5001` (HTTPS) | — | Synology DiskStation Manager admin UI · not UFW-gated (NAS is on LAN only) |
