...
**Purpose:** Client-side network control for the Samsung TV (192.168.50.175) via `tv_control.py`. Provides volume, power (WoL), keys, and now **YouTube search**. Not a systemd service — runs on-demand from sheepsoc conda env. See [Samsung TV Network Control runbook](../runbooks/wol-samsung-tv.md) for full procedure.
