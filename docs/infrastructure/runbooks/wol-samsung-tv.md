...
# Launches YouTube app (ID 111299001912 via run_app), navigates to search (KEY_UP/RIGHT/ENTER), uses `KEY_CLEAR` + sleep(0.5) to fully clear bar, sends text via send_text(), submits. Cleaner than prior backspace loop. Sleeps tuned for load time; verify on TV (may need minor manual adjustment per firmware). Tested successfully (token present, no pairing).
```
...
- **YouTube Search (`--youtube-search`):** `tv.run_app("111299001912")` to launch YouTube app, followed by navigation keys (`KEY_UP`, `KEY_RIGHT`, `KEY_ENTER` to reach search field), then single `tv.send_key("KEY_CLEAR")` + `sleep(0.5)` (TV has dedicated clear button that fully clears search bar in one step; confirmed and tested successfully), then `send_text(query)`, final `KEY_ENTER`. Sleeps (6s for app load, 1-2s between actions, 0.5s after clear) tuned for reliability. Prevents stale query text. Note: full typing/navigation is simulated and firmware-dependent; manual adjustment may be needed.
...
This runbook revised per schema.md section 6 update triggers for code change to TV control procedure (KEY_CLEAR refinement), runbook, services.md, schema.md, and known-issues.md. Updated purpose/How It Works/troubleshooting/usage to document current `KEY_CLEAR` + sleep(0.5) clear procedure (replaces prior backspace loop; TV dedicated button confirmed). All prior WoL/volume content preserved (reversible via prior git commit). Links follow section 4 rules exactly: relative paths from current location in `docs/infrastructure/runbooks/`, first-mention labels only per section, explicit relationship labels (**see also**, **runbook**), reciprocal links to affected pages (services.md#tv-control, topology.md#network-topology, index.md, known-issues.md#history-log, schema.md) where meaningful (no noise). Backlinks and interlinks verified — no broken links. No mkdocs build performed. Wiki is living record of current tv_control clear procedure per CLAUDE.md. Brief to Documentation agent covered the KEY_CLEAR refinement + successful test (token present, no pairing).
